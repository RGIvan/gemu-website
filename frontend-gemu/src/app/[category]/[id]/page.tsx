...
export async function generateMetadata({ params }: Props) {
  const product: EnrichedProduct = await getProduct(params.id);
...
const AllProducts = async ({ id }: { id: string }) => {
  ...
  const product: EnrichedProduct = await getProduct(id);
  const randomProducts: EnrichedProduct[] = await getRandomProducts(BigInt(id));
